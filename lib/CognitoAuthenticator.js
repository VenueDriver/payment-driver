...
class CognitoAuthenticator {
  ...
  async authenticate(parameters) {
    ...
    try {
      ...
      Debugger.info(["Authentication success."])
      Debugger.info(["Auth response: " + JSON.stringify(authResponse)])
    }
    catch (error) {
      Debugger.info(["Authentication error: "])
      Debugger.printError([error, error.stack]);
      throw error
    }
  ...
  async respondToAuthChallenge(lastResponse, username, new_password) {
    ...
    try {
      ...
      Debugger.info(["Changing password."])
      ...
      Debugger.info(["Challenge response success:"])
      Debugger.info([JSON.stringify(challengeResponseResponse)])
    }
    catch (error) {
      Debugger.info(["Challenge response error:"])
      Debugger.printError([error, error.stack])
    }
  ...
  async downloadPEM() {
    ...
    try {
      ...
      Debugger.printError(["Unable to download JWKs, fail the call: " + error])
      return false
    }
  }
  ...
  verifyJWTToken(token) {
    ...
    if (!pem) {
      Debugger.info(['Invalid access token'])
      return false
    }

    //Verify the signature of the JWT token to ensure it's really coming from the user pool.
    Debugger.info(["JWT verify."])
    try {
      jwt.verify(token, pem, { issuer: this.iss })
    }
    catch (error) {
      Debugger.printError(["Error verifying JWT signature: " + JSON.stringify(error)])
      throw error
    }

    Debugger.info(['The access token is valid.'])
    return true
  }
