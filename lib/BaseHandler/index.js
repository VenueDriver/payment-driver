...
class BaseHandler {
  ...
  constructor(name , options = {}){
    try {
      ...
      Debugger.printError(['Error in base handler constructor',err]);
  ...
  processEvent(){
    ...
    Debugger.debug(["this.base_url",this.base_url]);
  }
  ...
  async do(event,context){
    ...
    Debugger.info(['event: ' + JSON.stringify(event)])
    Debugger.info(['context: ' + JSON.stringify(context)])
    ...
    for(let i = 0; i < this._middlewares.length ; i++){
      ...
      try{
        ...
        Debugger.printError(["Middleware error",e]);
