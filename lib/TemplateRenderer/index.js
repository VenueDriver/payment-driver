...
class TemplateRenderer {
  ...
  async getRoutes(){
    ...
    Logger.debug(["TemplateRenderer.routes",this.routes]);
    return this.routes;
  }
  ...
  async render(page, parameters){
    ...
    Logger.debug(["Single route:",_single_route,"page",page,"template",template])
  ...
  async renderPartial(partial, parameters){
    ...
    Logger.debug(["Loading:",_single_route]);

    // READ SAID FILE AND STORE THE ONE VALUE
    this.partials[partial] = (await this.load(_single_route))[partial];

    Logger.debug(["index renderPartial result:",this.partials[partial]])
  ...
  parameters(parameters = {}){
    const defaults = {
      ...
      "date_format" : function(){
        return function(text,render){
          Logger.debug(["date_format",text])
          let [dateString,format,timezone] = render(text).split(',')
          Logger.info(["date_format",dateString,format,timezone])
