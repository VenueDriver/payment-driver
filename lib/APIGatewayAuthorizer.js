...
class APIGatewayAuthorizer {
   ...
   * is not logged in.
   */
  async getValidAuthenticationTokenFromCookie(event) {
    Logger.info(["Looking for authentication token..."])
    const authenticationToken = this.getAuthenticationTokenCookie(event)

    if (authenticationToken) {
      Logger.info(["Verifying authentication token from cookie: " + authenticationToken])
      const userPoolId = process.env.USER_POOL_ID
      const region = process.env.AWS_REGION
      const clientId = process.env.CLIENT_ID
      const authenticator = new CognitoAuthenticator(region, userPoolId, clientId)
      await authenticator.verifyCognitoToken(authenticationToken)
  ...
  /**
   * Get the "authentication_token" cookie from the HTTP headers in the API Gateway event.
   * @param {Object} event An event from API Gateway that was a parameter to an AWS Lambda function.
   * @returns {String} The value of the "authentication_token" cookie if it exists, and nothing if not.
   */
  getAuthenticationTokenCookie(event) {
    var cookies = [];
    for (const header of Object.keys(event.headers)) {
      if (header.toLowerCase() == 'cookie') {
        cookies = event.headers[header].split('; ')
      }
    }

    for (var cookie of cookies) {
      cookie = cookie.split('=')
      if (cookie[0] == 'authentication_token') {
        return cookie[1]
      }
    }
  }
