...
class TemplateRenderer {
  ...
  async getRoutes(){
    ...
    Debugger.debug(["TemplateRenderer.routes",this.routes]);
    return this.routes;
  }
  ...
  async render(page, parameters){
    ...
    Debugger.debug(["Single route:",_single_route,"page",page,"template",template])
  ...
  async renderPartial(partial, parameters){
    ...
    Debugger.debug(["Loading:",_single_route]);

    // READ SAID FILE AND STORE THE ONE VALUE
    this.partials[partial] = (await this.load(_single_route))[partial];

    Debugger.debug(["index renderPartial result:",this.partials[partial]])
  ...
  parameters(parameters = {}){
    const defaults = {
      ...
      "date_format" : function(){
        return function(text,render){
          Debugger.debug(["date_format",text])
          let [dateString,format,timezone] = render(text).split(',')
          Debugger.info(["date_format",dateString,format,timezone])

          if(timezone) return moment(dateString).tz(timezone).format(format)
          else return moment(dateString).format(format)
        }
      },
      "truncate" : function(){
        return function(text,render){
          let [ str = "" ,length = "max=120", trail = "" ] = render(text).split(",")
